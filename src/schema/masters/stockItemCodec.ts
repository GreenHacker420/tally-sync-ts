...
export const stockItemCodec: TallyCodec<StockItem> = {
  ...
  parse(node: Record<string, unknown>): StockItem {
    ...
    return {
      ...
      openingBalance: r.quantity("OPENINGBALANCE")?.value ?? r.number("OPENINGBALANCE") ?? 0,
      openingRate: r.rate("OPENINGRATE")?.value ?? r.number("OPENINGRATE"),
      openingValue: r.amountVal("OPENINGVALUE") ?? r.number("OPENINGVALUE"),
      closingBalance: r.quantity("CLOSINGBALANCE")?.value ?? r.number("CLOSINGBALANCE") ?? 0,
      closingRate: r.rate("CLOSINGRATE")?.value ?? r.number("CLOSINGRATE"),
      closingValue: r.amountVal("CLOSINGVALUE") ?? r.number("CLOSINGVALUE"),
